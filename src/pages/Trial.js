...
function Trial() {
    ...
    const submitTrialForm = async () => {
        // Validate all variables
        if (name === "" || email === "" ||  gender === "" || category === "" || category === "none") {
            alert("Please fill all the fields");
            return;
        }

        try{
            // Validate name as full name or halfname or without any number
            const namePattern = /^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$/;
            if (!namePattern.test(name)) {
                alert('Please enter a valid name');
                return;
            ...
            // API call to submit the trial form
            const response = await axios.post(`${apiUri}/submittrial`, {
                name: name,
                email: email,
                gender: gender,
                category: category,
